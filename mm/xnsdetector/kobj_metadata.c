...
#define MEM_INDUCED 13

extern long get_ts(void); // memorizer.c
...
/**
 * __htbl_enter() - increment recursion counter for entry into memorizer
 *
 ...
 */
static inline int __htbl_enter(void)
{
    return test_and_set_bit_lock(0,&inlt);
}

static __always_inline void __htbl_exit(void)
{
...
static struct memorizer_kobj **tbl_get_l1_entry(uint64_t addr)
{
	...
	l3e = htbl_l3_entry(&kobj_l3_tbl, addr);
	if (!*l3e)
		return NULL;
	l2e = htbl_l2_entry(*l3e, addr);
	if (!*l2e)
		return NULL;
	l1e = htbl_l1_entry(*l2e, addr);
	if (!*l1e)
		return NULL;
	return l1e;
}
...
/**
 * htbl_remove_kobj() --- remove object from the table
 ...
 */
struct memorizer_kobj * htbl_remove_kobj(uintptr_t addr)
{
...
inline struct memorizer_kobj * htbl_get_kobj(uintptr_t addr)
{
    struct memorizer_kobj **l1e = tbl_get_l1_entry(addr);
...
{
    unsigned long flags;
    struct memorizer_kobj *obj = htbl_get_kobj(addr);
...
/**
 * htbl_insert_kobj() - insert kobject into the lookup table
 ...
 * given kobj pointer.
 */
int __htbl_insert(uintptr_t ptr, size_t size, uintptr_t metadata)
{
	struct lt_l1_tbl **l2e;
	struct lt_l2_tbl **l3e;
	uint64_t l1_i = 0;
 ...
 */
size_t d = 0;
int htbl_insert_induced(void * ptr, size_t size)
{
    uintptr_t label = ((uintptr_t) MEM_INDUCED << ALLOC_CODE_SHIFT) |
        atomic_long_inc_return(&global_kobj_id);
    __htbl_insert((uintptr_t)ptr, size, label);
    return 1;
}

int htbl_insert_kobj(struct memorizer_kobj *kobj)
{
        return __htbl_insert(kobj->va_ptr, kobj->size, (uintptr_t)kobj);
}
