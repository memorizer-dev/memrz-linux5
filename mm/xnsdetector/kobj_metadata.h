...
#define htbl_l1_tbl_index(va)	(va & (LT_L1_ENTRIES - 1))
#define htbl_l2_tbl_index(va)	((va >> LT_L1_SHIFT) & (LT_L2_ENTRIES - 1))
#define htbl_l3_tbl_index(va)	((va >> LT_L2_SHIFT) & (LT_L3_ENTRIES - 1))
 ...
 */
static inline struct memorizer_kobj **htbl_l1_entry(struct lt_l1_tbl *l1_tbl,
		uintptr_t va)
{
	return &(l1_tbl->kobj_ptrs[htbl_l1_tbl_index(va)]);
}

static inline struct lt_l1_tbl **htbl_l2_entry(struct lt_l2_tbl *l2_tbl, uintptr_t
		va)
{
	return &l2_tbl->l1_tbls[htbl_l2_tbl_index(va)];
}

static inline struct lt_l2_tbl **htbl_l3_entry(struct lt_l3_tbl *l3_tbl, uintptr_t
		va)
{
	return &l3_tbl->l2_tbls[htbl_l3_tbl_index(va)];
}

static inline struct pid_obj * htbl_pid(struct lt_pid_tbl *pid_tbl,  uint32_t key)
{
...
struct memorizer_kobj * htbl_remove_kobj(uintptr_t va);
struct memorizer_kobj * htbl_get_kobj(uintptr_t va);
int htbl_insert_induced(void * vaddr, size_t size);
